...
class ChronosModel(BaseModel):
    ...
    def forecast(self):
        ...
        for id in datasets:
            ...
            # 2025 had 53 weeks, not 52. An extra week to predict!
            if id == 4:
                n_weeks_ahead = 68

            pred_dates  = pd.date_range(
                start=train_max,
                end = train_max + pd.Timedelta(weeks=n_weeks_ahead),
                freq="W"
            ).tolist()[1:] # exclude the first date (which is in train data)
    ...
    def _reshape_preds(
        ...
        df[self.cols_prediction] = df[self.cols_prediction].clip(lower=0)   # negative predictions (high uncertainty) are clipped
